...
for seq_record in listofSeqs:
    dnaSeq = seq_record.lower()
    if re.match(r'^atg', dnaSeq) and len(dnaSeq)%3 == 0:
...
##            col.append(round((col[1]/Total)*1000,3))
            ...
            col.append(round(col[1]/Sum, 3)) #Fraction
            col.append(round((col[1]/Total)*1000,3))
...
codons = []
fractions = []
for row in saccharomyces_cerevisiae:
    for col in row:
        codons.append(col[0])
        fractions.append(col[2])
listoffractions.append(fractions)

data = pd.DataFrame(listoffractions, columns = codons ,index=
["Saccharomyces Cerevisiae Regen"])
